...
class FrankaCubeStack(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.conveyor_default_vel=to_torch([0.5], device=self.device)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        franka_dof_friction=to_torch([0, 0, 0, 0, 0, 0, 0.01, 0.01, 0.01, 0.01], dtype=torch.float, device=self.device)
        ...
        for i in range(self.num_envs):
            ...
            self.gym.set_dof_target_velocity(env_ptr,target_dof_actor,0.5)
...
def compute_franka_reward(
    ...
    cubeA_color=states["cubeA_COLOR"]
    ...
    blue_table_box_pos = torch.tensor([-0.8,1.05,1.0], device=states["table_box_pos"].device)
    ...
    cubeA_lifted = ((cubeA_height - cubeA_size) > 0.08)
    lift_reward = cubeA_lifted

    # Use fixed_table_box_pos instead of states["table_box_pos"][:,:2]
    cubeA_to_table_box_distance = torch.norm(green_table_box_pos[:2] - states["cubeA_pos"][:, :2], dim=-1)

    distance_reward = (1 - torch.tanh(2.0 * cubeA_to_table_box_distance)) * cubeA_lifted
    cubeA_align_cubeB = (torch.norm(states["cubeA_pos"][:, :2] - green_table_box_pos[:2], dim=-1) < 0.2)
