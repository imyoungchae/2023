...
class FrankaCubeStack(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.conveyor_default_vel=to_torch([0.3], device=self.device)
        self.conveyor_default_pos=to_torch([0.6],device=self.device)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        red_ycb_asset_file= "urdf/ycb/061_foam_brick/061_foam_brick.urdf"
        ...
        if "asset" in self.cfg["env"]:
            ...
            red_ycb_asset_file = self.cfg["env"]["asset"].get("red_ycb", red_ycb_asset_file)
        ...
        franka_dof_stiffness = to_torch([10, 10, 10, 10, 10, 10, 800., 800.,800., 800.], dtype=torch.float, device=self.device)
        franka_dof_damping = to_torch([0, 0, 0, 0, 0, 0, 1.0e3, 1.0e3, 1.0e3, 1.0e3], dtype=torch.float, device=self.device)
        ...
        table_stand_pos_box3 = [-0.8, -0.45, 1.0 ]
        ...
        max_agg_bodies = num_franka_bodies + 6    # 1 for table, table stand, cubeA
        max_agg_shapes = num_franka_shapes + 6    # 1 for table, table stand, cubeA'
        ...
        for i in range(self.num_envs):
            ...
            self.gym.set_dof_target_velocity(env_ptr,target_dof_actor,0.3)

            self.table_stand_actor = self.gym.create_actor(env_ptr, table_stand_asset, table_stand_start_pose, "table_stand",
                                                      i, 1, 0)
            self.table_stand_actor2 = self.gym.create_actor(env_ptr, table_stand_asset, table_stand_start_pose2, "table_stand2",
                                                      i, 1, 0)
            self.table_stand_actor3 = self.gym.create_actor(env_ptr, table_stand_asset, table_stand_start_pose3, "table_stand3",
                                                      i, 1, 0)
    ...
    def post_physics_step(self):
        ...
        # debug viz
        if self.viewer != None:
            cam_pos = gymapi.Vec3(0.5,0.8,3.5)
...
def compute_franka_reward(
    ...
    red_table_box_pos=torch.tensor([-0.8, 0.35,1.0], device=states["table_box_pos"].device)
    green_table_box_pos = torch.tensor([-0.8, 0.7,1.0], device=states["table_box_pos"].device)
    blue_table_box_pos = torch.tensor([-0.8, -0.45, 1.0], device=states["table_box_pos"].device)
    ...
    # Use fixed_table_box_pos instead of states["table_box_pos"][:,:2]
    cubeA_to_table_box_distance = torch.norm(blue_table_box_pos[:2] - states["cubeA_pos"][:, :2], dim=-1)

    distance_reward = (1 - torch.tanh(2.0 * cubeA_to_table_box_distance)) * cubeA_lifted
    cubeA_align_cubeB = (torch.norm(states["cubeA_pos"][:, :2] - blue_table_box_pos[:2], dim=-1) < 0.295)
    cubeA_on_cubeB = torch.abs(cubeA_height - target_height) < 0.09
    gripper_away_from_cubeA = (d > 0.055)

    goal_reward = cubeA_align_cubeB & cubeA_on_cubeB & gripper_away_from_cubeA
    #goal_reward = cubeA_align_cubeB & gripper_away_from_cubeA

    reset_condition = (states["cubeA_pos"][:, 1] > 0.7)
    # end penalty
    end_penalty = (states["cubeA_pos"][:, 1] > 0.7)
    ...
    print("distance_reward",distance_reward[0])
    print("cubeA_align_cubeB",cubeA_align_cubeB[0])
    print("torch.norm(states[cubeA_pos][:, :2] - green_table_box_pos[:2], dim=-1)",torch.norm(states["cubeA_pos"][:, :2][0] - blue_table_box_pos[:2], dim=-1))
    print("gripper_away_from_cubeA",gripper_away_from_cubeA[0])
    print("states[cubeA_pos][:, 1]",states["cubeA_pos"][:, 1][0])
    print("torch.abs(cubeA_height - target_height)",torch.abs(cubeA_height - target_height)[0])
    print("goal reward",goal_reward[0])

    rewards = torch.where(
        goal_reward,
        reward_settings["r_goal"] * goal_reward,
        reward_settings["r_dist_scale"] * dist_reward + reward_settings["r_lift_scale"] * lift_reward +
        reward_settings["r_distance_reward"] * distance_reward - reward_settings["r_endpenalty"] * end_penalty)
    reset_buf = torch.where((progress_buf >= max_episode_length - 1) | (reset_condition > 0) | (
                goal_reward > 0), torch.ones_like(reset_buf), reset_buf)
