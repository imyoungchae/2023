...
class FrankaCubeStack(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.cfg["env"]["numObservations"] = 23 if self.control_type == "osc" else 27
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        franka_asset_file = "urdf/ur10_sy/urdf/ur10_gripper.urdf"
        conveyor_asset_file = "urdf/cartpole.urdf"
        ...
        franka_dof_stiffness = to_torch([0, 0, 0, 0, 0, 0, 5000., 5000.,5000., 5000.], dtype=torch.float, device=self.device)
        ...
        cubeA_color = gymapi.Vec3(0.82,0.93,0.09)
        ...
        for i in range(self.num_franka_dofs):
            ...
            if self.physics_engine == gymapi.SIM_PHYSX:
                ...
                franka_dof_props['stiffness'][i] = 800.0
        ...
        for i in range(self.num_envs):
            ...
            self.gym.set_dof_target_velocity(env_ptr,target_dof_actor,0.2)
            ...
            # Create cubes
            self._cubeA_id = self.gym.create_actor(env_ptr, cubeA_asset, cubeA_start_pose, "cubeA", i, 4, 0)
    ...
    def _update_states(self):
        self.states.update({
            ...
            "conv_pos": self._conveyor_dof_pos[:,:],
    ...
    def compute_observations(self):
        self._refresh()
        obs = ["cubeA_quat", "cubeA_pos", "eef_pos", "eef_quat", "cubeA_vel"]
    ...
    def reset_idx(self, env_ids):
        ...
        #print("self._pos_control.shape",self._pos_control.shape)
        #print("len(multi_env_ids_int32)",len(multi_env_ids_int32))
    ...
    def pre_physics_step(self, actions):
        ...
        u_fingers[:, 2:] = torch.where(u_gripper_expanded[:, 2:] >= 0.0, torch.tensor([[0.6, 0.6]], device=self.device),
                                        torch.tensor([[-0.6, -0.6]], device=self.device))
...
def compute_franka_reward(
    ...
    # Check if cubeA_pos has reached target_height
    goal_reward = (states["cubeA_pos"][:, 2] >= target_height).float()

    # Check if cubeA_pos[:,1] is greater than 1.1
    reset_condition = states["cubeA_pos"][:, 1] > 1.1

    # Apply end penalty if reset_condition is True
    end_penalty = reset_condition.float()
    ...
    rewards = torch.where(
        ...
        reward_settings["r_dist_scale"] * dist_reward + reward_settings["r_lift_scale"] * lift_reward - reward_settings[
            "r_endpenalty"] * end_penalty,
    )
    # Compute resets
    reset_buf = torch.where((progress_buf >= max_episode_length - 1) | reset_condition, torch.ones_like(reset_buf), reset_buf)
